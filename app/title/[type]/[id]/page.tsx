...
export default async function TitlePage({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c") }}/><section className="detail-hero">{item.backdrop_path && <Image className="detail-backdrop" src={image(item.backdrop_path, "original")} alt="" fill priority sizes="100vw"/>}<div className="detail-scrim"/><div className="detail-copy"><span className="eyebrow">{type === "movie" ? "Film" : "Series"} · {yearOf(item)}</span><h1>{title}</h1><div className="hero-meta"><span>★ {item.vote_average.toFixed(1)}</span>{item.genres?.slice(0, 3).map(g => <span key={g.id}>{g.name}</span>)}</div>{facts.length > 0 && <div className="detail-facts">{facts.map(fact => <span key={fact}>{fact}</span>)}</div>}<p>{item.overview}</p>{item.tagline && <p className="tagline"><em>“{item.tagline}”</em></p>}</div></section><div className="detail-grid"><div>{trailer ? <><h2 className="subhead">Official trailer</h2><div className="trailer"><iframe src={`https://www.youtube-nocookie.com/embed/${trailer}`} title={`${title} official trailer`} allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowFullScreen loading="lazy"/></div></> : <div className="trailer-empty"><span className="eyebrow">Trailer</span><h2 className="subhead">No official trailer listed</h2></div>}{cast.length > 0 && <><h2 className="cast-heading">Top cast</h2><div className="cast-list">{cast.map(person => <span key={person.id}>{person.name}{person.character ? ` · ${person.character}` : ""}</span>)}</div></>}</div><WatchProviders data={providers} region={defaultRegion}/></div>{item.recommendations?.results?.length ? <div className="home-shell"><MediaRow eyebrow="If you liked this" title="Keep watching" items={item.recommendations.results} type={type}/></div> : null}</>;
}
